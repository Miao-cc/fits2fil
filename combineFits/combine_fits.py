...
#gainAA/gainBB:  1.4110690554634382 -0.7071839332424865
fileroot=sys.argv[1]
Fitslist=sys.argv[2]
Ra = sys.argv[3]
Dec = sys.argv[4]
gainDiff = float(sys.argv[5])
baseLineDiff = float(sys.argv[6])
...
for line in open(Fitslist):
    ...
    data=hdulist[1][:]
    pol_AA = data['DATA'][:,:,0,:,:]
    pol_BB = np.array(data['DATA'][:,:,1,:,:]*gainDiff+baseLineDiff, dtype=np.uint8)
