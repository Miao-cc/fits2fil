...
sifting.short_period = 0.0011
# Longest period candidates to consider (s)
sifting.long_period = 1.50
...
if len(cands):
    ...
    cands.sort(key=attrgetter('p'), reverse=True)
...
if (fold):
    cands.sort(key=attrgetter('sigma'), reverse=True)
    #cands.sort(key=attrgetter('p'), reverse=True)
    candlist = cands.get_all_cands()

    for num, cand in enumerate(candlist):
        filename = cand.filename
        candnum = cand.candnum
        #print(num, filename, candnum)
        cmd = "prepfold -noxwin -nosearch -accelcand %i -accelfile %s.cand  %s.dat" %(candnum, filename, filename.split("_ACCEL")[0])
        os.system(cmd)
